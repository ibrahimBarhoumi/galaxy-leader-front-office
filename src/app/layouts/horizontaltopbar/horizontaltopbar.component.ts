...
export class HorizontaltopbarComponent implements OnInit, AfterViewInit {
 ...
 ngOnInit(): void {
  ...
  this.roles = tokenPayload['authorities'];
  ...
  this.configData = {
    ...
    if (val.length === 0) {
      if (this.flagvalue === undefined) {
        this.valueset = 'assets/images/flags/fr.jpg';
      }
    } else {
   ...
   */
  logout() {
    this.subscribtions = this.loginService.logout().subscribe();
  ...
  initialize(): void {
    ...
    this.configurationService.list(new QueryOptions(this.page - 1, this.pageSize)).subscribe((data) => {
      this.configurationData = data['content'];

      this.menuItems.find(e=>e.id===4).subItems.find(e=>e.id==9).subItems = []
     this.configurationData.forEach(element => {

      this.menuItems.find(e=>e.id===4).subItems.find(e=>e.id==9).subItems.push({
        id : element.id ,
        roles:['ROLE_ADMIN'],
        label : element.label ,
        link : '/configvalue/'+ element.id ,
